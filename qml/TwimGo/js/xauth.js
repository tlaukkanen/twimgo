...
function XAuth() {
    ...
    var OAUTH_CONSUMER_TOKEN = "rUeGDPrcxbYN6r63JA39Kg";
    var OAUTH_CONSUMER_SECRET = "EoNvzuQ5gz0LhbbssK4B9e9REdlN52ngl6YoN6eUqrs";
    ...
    this.setOAuthVerifier = function(ver) {
        verifier = ver;
    }
    ...
    function doWebRequest(method, url, callback, errorCallback) {
        ...
        doc.onreadystatechange = function() {
            ...
            if (doc.readyState == XMLHttpRequest.HEADERS_RECEIVED) {
                ...
                if(status!=200 && url.indexOf("help/test.json")<0) {
                    if(shortURL.length>64) {
                        shortURL = shortURL.substring(0,64) + "..";
                    }
                    var errorDesc = doc.responseText;
                ...
                if(status!=200 && url.indexOf("help/test.json")<0) {
                    ...
                    if(shortURL.length>60) {
                        shortURL = shortURL.substring(0,60) + "..";
                    }
                    var errorDesc = doc.responseText;
                    if(typeof(errorDesc)=="undefined") {
                        errorDesc = "";
    ...
    this.generateSignatureBase = function(url,consumerKey,token,tokenSecret,httpMethod,timeStamp,nonce,signatureType) {
        ...
        if(verifier!="") {
            parameters.push( [OAuthVerifier, verifier] );
        }
